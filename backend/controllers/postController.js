...
const deletePost = async (req, res) => {
    try {
        ...
        if(post.img) {
            ...
            await cloudinary.uploader.destroy(imgId); //delete the img on cloudinary
        }
...
const replyToPost = async (req, res) => {
    try {
        ...
        res.status(200).json(reply); //respond to request
...
const getUserPosts = async (req, res) => {
    const { username } = req.params; //username of the user we want to get posts from
    try {
        ...
        const posts = await Post.find({postedBy: user._id}).sort({createdAt: -1}); //find posts with a postedBy of our user's id and sort the results in descending order based on createdAt
